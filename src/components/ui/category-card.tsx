...
const CategoryCard = ({ category, onClick, className = "" }: CategoryCardProps) => {
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.95 }}
      onClick={handleClick}
      className={`bg-[#0D0A1D] border-none rounded-lg p-4 text-center cursor-pointer hover:border hover:!border-primary/50 transition-all duration-200 shadow-lg hover:shadow-xl ${className}`}
    >
      {/* Category Image */}

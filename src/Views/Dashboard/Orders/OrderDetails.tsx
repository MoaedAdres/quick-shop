    ...
    <RFlex className="flex-col h-full pb-20 md:pb-0">
      ...
      <div className="bg-card border-b border-border p-4">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-semibold text-foreground">Order #{order.id}</h1>
        </div>
      </div>
        ...
        <div className="p-4 space-y-6">
          <div className="bg-card border border-border rounded-lg p-6">
            ...
            <div className="flex justify-between items-start mb-6">
              <div>
                <h2 className="text-2xl font-bold text-foreground">Order #{order.id}</h2>
            ...
            <div className="grid md:grid-cols-2 gap-8">
              ...
              <div>
                <h3 className="font-semibold text-lg mb-4 text-foreground">Shipping Information</h3>
                <div className="space-y-2">
                  <p><span className="text-muted-foreground">Name:</span> <span className="text-foreground">{order.full_name}</span></p>
                  <p><span className="text-muted-foreground">Address:</span> <span className="text-foreground">{order.address}</span></p>
                  {order.address2 && (
                    <p><span className="text-muted-foreground">Address 2:</span> <span className="text-foreground">{order.address2}</span></p>
                  )}
                  <p><span className="text-muted-foreground">City:</span> <span className="text-foreground">{order.city}</span></p>
                  <p><span className="text-muted-foreground">Province:</span> <span className="text-foreground">{order.province}</span></p>
                  <p><span className="text-muted-foreground">Country:</span> <span className="text-foreground">{order.country}</span></p>
                  <p><span className="text-muted-foreground">ZIP:</span> <span className="text-foreground">{order.zip}</span></p>
                  <p><span className="text-muted-foreground">Phone:</span> <span className="text-foreground">{order.mobile_no}</span></p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  <p>
                    ...
                    <span className="text-foreground">{order.payment_method?.toUpperCase()}</span>
                  </p>
                  <p>
                    <span className="text-muted-foreground">Payment Reference:</span>{" "}
                    <span className="text-foreground">{order.payment_reference}</span>
                  </p>
                  <p>
                    <span className="text-muted-foreground">Payment Status:</span>{" "}
                    <span className="text-foreground">{order.paid_at ? "Paid" : "Pending"}</span>
                  </p>
                  {order.paid_at && (
                    <p>
            ...
            <div className="mt-8 border-t border-border pt-6">
              <div className="w-full max-w-sm ml-auto space-y-2">
                ...
                <div className="flex justify-between font-semibold text-lg pt-2 border-t border-border">
                  ...
                  <span className="text-foreground">${order.total_price?.toFixed(2)}</span>
                </div>
              </div>
            </div>

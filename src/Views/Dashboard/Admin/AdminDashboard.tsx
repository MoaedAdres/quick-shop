...
const AdminDashboard = () => {
  ...
  const { data: referralSettings, isLoading: referralLoading } =
    useGetReferralSettings();
  const { data: tappingSettings, isLoading: tappingLoading } =
    useGetTappingSettings();

  const updateReferralMutation = useUpdateReferralSettings();
  const updateTappingMutation = useUpdateTappingSettings();

  // Form states - using strings to allow empty values
  ...
  React.useEffect(() => {
    if (referralSettings?.data) {
      setReferralForm({
        reward_amount: String(parseFloat(referralSettings.data.reward_amount)),
      });
    }
  }, [referralSettings]);
  ...
  React.useEffect(() => {
    if (tappingSettings?.data) {
      setTappingForm({
        points_per_tap: String(tappingSettings.data.points_per_tap),
        taps_for_reward: String(tappingSettings.data.taps_for_reward),
        reward_amount: String(tappingSettings.data.reward_amount),
        daily_tap_limit: String(tappingSettings.data.daily_tap_limit),
      });
    }
  }, [tappingSettings]);
  ...
  const handleNumberInput = (value: string) => {
    // Allow empty string, numbers, and decimal point for floating point numbers
  ...
  // Helper function to handle integer input changes
  ...
  const handleReferralUpdate = async () => {
    try {
      ...
      await updateReferralMutation.mutateAsync(payload);
  ...
  const handleTappingUpdate = async () => {
    try {
      const payload = {
        points_per_tap: parseInt(tappingForm.points_per_tap) || 1,
        taps_for_reward: parseInt(tappingForm.taps_for_reward) || 5,
        reward_amount: parseInt(tappingForm.reward_amount) || 10,
        daily_tap_limit: parseInt(tappingForm.daily_tap_limit) || 100,
      };
      await updateTappingMutation.mutateAsync(payload);
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">
              Manage your Quick Shop platform
            </p>
          </div>
          <motion.button
              ...
              {ordersLoading ? (
                ...
                <div className="space-y-3 max-h-80 overflow-y-auto">
                  ...
                  {(!recentOrders?.data?.results ||
                    recentOrders.data.results.length === 0) && (
            ...
            >
              ...
              {usersLoading ? (
                ...
                <div className="space-y-3 max-h-80 overflow-y-auto">
                  ...
                  {(!recentUsers?.data?.results ||
                    recentUsers.data.results.length === 0) && (
                    <p className="text-center text-muted-foreground py-8">
                      No recent users
                    </p>
                  )}
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      onBlur={(e) => {
                        // On blur, ensure we have a valid number or default to 0
                        if (
                          e.target.value === "" ||
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const value = handleIntegerInput(e.target.value);
                          setTappingForm({
                            ...tappingForm,
                            points_per_tap: value,
                          });
                        }}
                        onBlur={(e) => {
                          if (
